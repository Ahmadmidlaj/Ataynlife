...
function App() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#fbfaf5] text-[#17382f]">
      ...
      <header className="sticky top-0 z-30 border-b border-[#153d3420] bg-[#fbfaf5]/95 backdrop-blur-md">
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 lg:px-8">
          ...
          <div className="flex items-center gap-1 sm:gap-2"><label className="hidden h-10 items-center gap-2 rounded-full border border-[#214d4020] bg-white px-3.5 text-[#567166] md:flex"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-36 bg-transparent text-[14px] outline-none placeholder:text-[#768c84]" placeholder="Find a favourite" aria-label="Search products" /></label><button onClick={() => setIsCartOpen(true)} className="relative grid size-10 place-items-center rounded-full transition-colors hover:bg-[#e8efe9]" aria-label="Open basket"><ShoppingBag size={19} />{itemCount > 0 && <span className="absolute -right-0.5 -top-0.5 grid min-w-5 place-items-center rounded-full bg-[#c5761e] px-1 text-[11px] font-bold leading-5 text-white">{itemCount}</span>}</button><button onClick={() => setIsMenuOpen((open) => !open)} className="grid size-10 place-items-center rounded-full hover:bg-[#e8efe9] lg:hidden" aria-label="Toggle navigation menu">{isMenuOpen ? <X size={21} /> : <Menu size={21} />}</button></div>
        </div>
        {isMenuOpen && <div className="border-t border-[#153d3415] bg-[#fbfaf5] px-5 py-4 lg:hidden"><div className="mb-4 flex h-11 items-center gap-2 rounded-full border border-[#214d4020] bg-white px-4 text-[#567166] md:hidden"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-[14px] outline-none" placeholder="Search the collection" aria-label="Search products" /></div><nav className="grid gap-3 text-[15px] font-medium"><a href="#shop" onClick={() => setIsMenuOpen(false)}>Shop</a><a href="#curated" onClick={() => setIsMenuOpen(false)}>Curated boxes</a><a href="#how-it-works" onClick={() => setIsMenuOpen(false)}>How it works</a><a href="#our-promise" onClick={() => setIsMenuOpen(false)}>Our promise</a><a href="#contact" onClick={() => setIsMenuOpen(false)}>Contact</a></nav></div>}
      </header>

      <section id="top" className="relative isolate min-h-[620px] overflow-hidden bg-[#0f3a2f]"><img src={imageUrls.tropical} alt="Fresh tropical fruits arranged in a colourful still life" className="absolute inset-0 -z-20 h-full w-full object-cover object-[center_45%]" /><div className="absolute inset-0 -z-10 bg-gradient-to-r from-[#0b3028]/95 via-[#0b3028]/74 to-[#0b3028]/20" /><div className="absolute inset-x-0 bottom-0 -z-10 h-2/5 bg-gradient-to-t from-[#0b3028]/65 to-transparent" /><div className="mx-auto flex min-h-[620px] max-w-7xl items-end px-5 pb-14 pt-20 lg:px-8 lg:pb-20"><div className="max-w-2xl text-[#fffdf4]"><div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#f4d998]/35 bg-[#153d34]/65 px-3.5 py-2 text-[12px] font-semibold tracking-[0.08em] text-[#f8d988] backdrop-blur-sm"><Sparkles size={14} /> THE SEASONAL EDIT</div><h1 className="font-display text-[50px] leading-[0.96] tracking-[-0.045em] sm:text-[66px] lg:text-[76px]">Naturally more <em className="font-normal text-[#f5cd75]">wonderful.</em></h1><p className="mt-6 max-w-xl text-[17px] leading-7 text-[#f7f1dc]/86 sm:text-[18px]">Shop fresh fruit online, rare exotic produce, premium dates and nourishing dry fruits—chosen with a discerning eye and sent out beautifully.</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><a href="#shop" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#f4c96f] px-6 text-[14px] font-bold text-[#15382f] transition hover:-translate-y-0.5 hover:bg-[#ffe09a]">Shop the selection <ArrowRight size={17} /></a><a href="#our-promise" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-white/45 px-6 text-[14px] font-bold text-white transition hover:bg-white/10">Why Atayinlife <ChevronRight size={17} /></a></div></div></div></section>
      ...
      <div className="fixed bottom-5 right-5 z-40 flex flex-col items-end gap-3">
        ...
        <button onClick={startGeneralConversation} className="group inline-flex size-[54px] items-center justify-center rounded-full bg-[#25D366] text-[#073a18] shadow-[0_12px_30px_rgba(12,90,44,.35)] transition hover:-translate-y-0.5 hover:bg-[#49ec86] sm:w-auto sm:gap-2 sm:px-5" aria-label="Chat with Atayinlife on WhatsApp"><MessageCircle size={23} strokeWidth={2.3} /><span className="hidden text-[14px] font-bold sm:inline">WhatsApp</span></button>
      </div>
