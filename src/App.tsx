...
function OrderDetailsDialog({ isOpen, details, subtotal, onClose, onBackToBasket, onChange, onSubmit }: { isOpen: boolean; details: OrderDetails; subtotal: number; onClose: () => void; onBackToBasket: () => void; onChange: (field: keyof OrderDetails, value: string) => void; onSubmit: () => void }) {
  ...
  return <div className="fixed inset-0 z-[60] flex items-end justify-center bg-[#0b2c25]/45 p-2 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="order-details-title">
    <button type="button" onClick={onClose} className="absolute inset-0 cursor-default" aria-label="Close delivery details form" />
    <form onSubmit={(event) => { event.preventDefault(); onSubmit() }} className="relative flex max-h-[calc(100dvh-1rem)] w-full max-w-lg flex-col overflow-hidden rounded-[1.6rem] bg-[#fbfaf5] shadow-2xl sm:max-h-[min(720px,calc(100dvh-2.5rem))] sm:rounded-[1.8rem]">
      <div className="shrink-0 border-b border-[#173d3315] px-6 pb-4 pt-5 sm:px-8 sm:pt-7">
        <button type="button" onClick={onClose} className="absolute right-5 top-4 grid size-10 place-items-center rounded-full text-[#5d756b] transition hover:bg-[#e9f0e9] hover:text-[#17382f] sm:right-7 sm:top-6" aria-label="Close delivery details form"><X size={21} /></button>
        <p className="text-[12px] font-bold tracking-[0.12em] text-[#af6e20]">ALMOST THERE</p>
        <h2 id="order-details-title" className="mt-2 pr-10 font-display text-[31px] leading-none tracking-[-0.04em] text-[#143b31] sm:text-[34px]">Where should we send it?</h2>
        <p className="mt-2 text-[14px] leading-6 text-[#60776d] sm:text-[15px]">A few details help us confirm your order quickly on WhatsApp.</p>
      </div>

      <div className="min-h-0 flex-1 touch-pan-y overflow-y-auto overscroll-contain px-6 py-5 sm:px-8 sm:py-6">
        <div className="grid gap-4">
          <label className="grid gap-1.5 text-[14px] font-bold text-[#294f44]">Your name<input required autoComplete="name" value={details.name} onChange={(event) => onChange('name', event.target.value)} className="h-12 rounded-xl border border-[#173d3320] bg-white px-4 text-[16px] font-medium text-[#17382f] outline-none transition placeholder:text-[#879890] focus:border-[#1d6953] focus:ring-2 focus:ring-[#1d6953]/15" placeholder="Name for the order" /></label>
          <label className="grid gap-1.5 text-[14px] font-bold text-[#294f44]">Delivery area<input required autoComplete="address-level2" value={details.deliveryArea} onChange={(event) => onChange('deliveryArea', event.target.value)} className="h-12 rounded-xl border border-[#173d3320] bg-white px-4 text-[16px] font-medium text-[#17382f] outline-none transition placeholder:text-[#879890] focus:border-[#1d6953] focus:ring-2 focus:ring-[#1d6953]/15" placeholder="Locality, city or pincode" /></label>
          <label className="grid gap-1.5 text-[14px] font-bold text-[#294f44]">Preferred delivery time<input required value={details.deliveryTime} onChange={(event) => onChange('deliveryTime', event.target.value)} className="h-12 rounded-xl border border-[#173d3320] bg-white px-4 text-[16px] font-medium text-[#17382f] outline-none transition placeholder:text-[#879890] focus:border-[#1d6953] focus:ring-2 focus:ring-[#1d6953]/15" placeholder="For example, today after 6 PM" /></label>
          <label className="grid gap-1.5 text-[14px] font-bold text-[#294f44]">Anything else? <span className="font-medium text-[#70857d]">Optional</span><textarea value={details.notes} onChange={(event) => onChange('notes', event.target.value)} rows={2} className="resize-none rounded-xl border border-[#173d3320] bg-white px-4 py-3 text-[16px] font-medium text-[#17382f] outline-none transition placeholder:text-[#879890] focus:border-[#1d6953] focus:ring-2 focus:ring-[#1d6953]/15" placeholder="Gift message, landmarks or special requests" /></label>
        </div>
      </div>

      <div className="shrink-0 border-t border-[#173d3315] bg-[#f4f0e4] px-6 py-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:px-8">
        <div className="flex items-center justify-between gap-4"><button type="button" onClick={onBackToBasket} className="text-[14px] font-bold text-[#356154] transition hover:text-[#a7661e]">← Edit basket</button><p className="whitespace-nowrap text-[14px] font-bold text-[#294f44]">Total <span className="ml-1 font-display text-[25px] text-[#103d31]">{currency(subtotal)}</span></p></div>
        <button type="submit" className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-[#25D366] px-5 text-[14px] font-bold text-[#063816] transition hover:-translate-y-0.5 hover:bg-[#38e878]"><FaWhatsapp size={20} /> Send order on WhatsApp</button>
      </div>
    </form>
  </div>
}
